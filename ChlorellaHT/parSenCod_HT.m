        ...
        for j =1:it1
            theta_0(:,j)=range_Stoech(:,1)'+rand(1,length(range_Stoech(:,1))).*(range_Stoech(:,2)-range_Stoech(:,1))';
             for i = 1:it2
                ...
                model_analysis = changeRxnMets(model_analysis,model_analysis.mets(Met_ID),model_analysis.mets(Met_ID),'Biomass_Cvu_hetero-',theta_0(:,j));
                out=optimizeCbModel(changeObjective(model_analysis,'Biomass_Cvu_hetero-'));
                growth{k}(i,j)=out.f;
                co2_uptake{k}(i,j)=out.full(strmatch('EX_glc-A(e)',model_analysis.rxns));
             end
        end
