...
eliminateMets = {'h_c','pi_c','atp_c','h2o_c','starch','dgtp_c','gtp_c','datp_c','chla_u','chlb_u','adp_c'};
...
for f = 1:length(fieldNames)
    ...
    if ~isempty(group)
        ...
        groupCost = ATPcost(model,Met(group),'ATPM(NGAM)','adp_c');
        ...
        title('Sensitivity')
        text(groupCost+0.1,groupSens,Met2(group),'FontSize')%,8*2)
        xlabel('ATP cost')
        ylabel('\Delta\mu/\Deltap')
        ...
        title('Sensitivity')
        text(groupConectivity+0.1,groupSens,Met2(group),'FontSize')%,8*2)
        xlabel('Conectivity')
        ylabel('\Delta\mu/\Deltap')
        % set(gca,'FontSize',8*2)
        ...
        if ~isempty(strmatch('allMets',groupname))
            ...
            save('3D_HT.mat','groupConectivity','groupCost','groupSens','groupCod','metTypes','group','Met','growth_sens_vector');
        end
