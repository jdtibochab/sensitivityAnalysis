...
% groupCost = biosyntheticCost(model,Met(group),0.057,'EX_co2_e'); % Pt
...
title('Sensitivity')
text(Cost,Sens,met_an,'FontSize',8)
xlabel('A_{CO_2}')
...
met_an = {'AA';'Glu';'TAG';'DAG';'MAG';'PE';'PG';'AS';'DGTS';'DGDG';'CB';'N';'ATP'};

AAsens = mean(growth_sens(AA_mets));
    AAsens_dev = std(growth_sens(AA_mets));
Alasens = growth_sens(strmatch('glu__L_c',Met));
    Alasens_dev = 0;
AAcod = mean(growth_cod(AA_mets));
    AAcod_dev = std(growth_cod(AA_mets));
Alacod = growth_cod(strmatch('glu__L_c',Met));
    Alacod_dev = 0;

Sens = [AAsens,Alasens,TAGsens,DAGsens,MAGsens,Pesens,Pgsens,Assens,Dgtssens,Dgdgsens,CBsens,Nucsens,ATPsens];
    Sens_dev = [AAsens_dev,Alasens_dev,TAGsens_dev,DAGsens_dev,MAGsens_dev,Pesens_dev,Pgsens_dev,Assens_dev,Dgtssens_dev,Dgdgsens_dev,CBsens_dev,Nucsens_dev,ATPsens_dev];
Cod = [AAcod,Alacod,TAGcod,DAGcod,MAGcod,Pecod,Pgcod,Ascod,Dgtscod,Dgdgcod,CBcod,Nuccod,ATPcod];
    Cod_dev = [AAcod_dev,Alacod_dev,TAGcod_dev,DAGcod_dev,MAGcod_dev,Pecod_dev,Pgcod_dev,Ascod_dev,Dgtscod_dev,Dgdgcod_dev,CBcod_dev,Nuccod_dev,ATPcod_dev];
Cost = [AAcost,TAGcost,DAGcost,MAGcost,Pecost,Pgcost,Ascost,Dgtscost,Dgdgcost,CBcost,Nuccost,ATPcost];
    Cost_dev = [AAcost_dev,TAGcost_dev,DAGcost_dev,MAGcost_dev,Pecost_dev,Pgcost_dev,Ascost_dev,Dgtscost_dev,Dgdgcost_dev,CBcost_dev,Nuccost_dev,ATPcost_dev];

AAcon = mean(groupConectivity(AA_mets));
    AAcon_dev = std(groupConectivity(AA_mets));
Alacon = groupConectivity(strmatch('glu__L_c',Met));
    Alacon_dev = 0;
...
ATPcon = groupConectivity(strmatch('atp_c',Met));
    ATPcon_dev = 0;

Con = [AAcon;Alacon;TAGcon;DAGcon;MAGcon;Pecon;Pgcon;Ascon;Dgtscon;Dgdgcon;CBcon;Nuccon;ATPcon]';
    Con_dev = [AAcon_dev,Alacon_dev,TAGcon_dev,DAGcon_dev,MAGcon_dev,Pecon_dev,Pgcon_dev,Ascon_dev,Dgtscon_dev,Dgdgcon_dev,CBcon_dev,Nuccon_dev,ATPcon_dev];
...
% xlim([0,20])
title('Parameter codependence')
text(Con+0.15,Cod+1.5e-6,met_an,'FontSize',8)
xlabel('Conectivity')
ylabel('\Delta(\Delta\mu/\Deltap)')
x0=10;
y0=10;
