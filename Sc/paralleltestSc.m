...
[Stoich,Met] = xlsread('ScBOF.xlsx');
Stoich(1,:) = [];
Stoich = Stoich/1000;

% model_an = changeRxnBounds(model_an,'EX_glc__D_e',-0.01,'l');
% model_an = changeRxnBounds(model_an,'EX_nh4_e',-10,'l');

[model_an,prodCapMet,notProdCapMet,Stoich2,theta_0] = metTest(model_an,Met,Stoich);
Stoich = Stoich2;
...
%     model_temp.lb(strmatch('EX_no3(e)',model_temp.rxns)) = rN(i);
    ...
    ro = abs(out{i}.x(strmatch('EX_etoh_e',model_temp.rxns)))*2+...
        abs(out{i}.x(strmatch('EX_for_e',model_temp.rxns)))*1;
