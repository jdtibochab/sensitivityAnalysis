...
for f = 1:length(fieldNames)
    ...
    if ~isempty(group)
        ...
        groupCost = ATPcost(model,mets(group),'227','adp_c');
        ...
        width=1000;
        height=800;
        set(gca,'FontSize')%,8*2)growth_sens_vector
        set(gcf,'units','points','position',[x0,y0,width,height])
        ...
        if ~isempty(strmatch('allMets',groupname))
            ...
            save('3D_Yl.mat','groupConectivity','groupCost','groupSens','groupCod','metTypes','group','Met','growth_sens_vector');
        end
    end
end
