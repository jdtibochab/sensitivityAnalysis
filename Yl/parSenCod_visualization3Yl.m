...
groupCost = [44	40	40	29	28	44	46	55	3	6	4	4	3	5	5	2	6	6	6	6	5	9	5	3	4	11	9	5	5	6	5];
...
AA_mets(strmatch('ala__L_c',Met(AA_mets))) = [];

met_an = {'AA';'Ala';'TAG';'DAG';'MAG';'PE';'PG';'AS';'DGTS';'DGDG';'CB';'N'};

AAsens = mean(growth_sens(AA_mets));
    AAsens_dev = std(growth_sens(AA_mets));
Alasens = growth_sens(strmatch('ala__L_c',Met));
    Alasens_dev = 0;
AAcod = mean(growth_cod(AA_mets));
    AAcod_dev = std(growth_cod(AA_mets));
Alacod = growth_cod(strmatch('ala__L_c',Met));
    Alacod_dev = 0;

Sens = [AAsens,Alasens,TAGsens,DAGsens,MAGsens,Pesens,Pgsens,Assens,Dgtssens,Dgdgsens,CBsens,Nucsens];
    Sens_dev = [AAsens_dev,Alasens_dev,TAGsens_dev,DAGsens_dev,MAGsens_dev,Pesens_dev,Pgsens_dev,Assens_dev,Dgtssens_dev,Dgdgsens_dev,CBsens_dev,Nucsens_dev];
Cod = [AAcod,Alacod,TAGcod,DAGcod,MAGcod,Pecod,Pgcod,Ascod,Dgtscod,Dgdgcod,CBcod,Nuccod];
    Cod_dev = [AAcod_dev,Alacod_dev,TAGcod_dev,DAGcod_dev,MAGcod_dev,Pecod_dev,Pgcod_dev,Ascod_dev,Dgtscod_dev,Dgdgcod_dev,CBcod_dev,Nuccod_dev];


AAcon = mean(groupConectivity(AA_mets));
    AAcon_dev = std(groupConectivity(AA_mets));
Alacon = groupConectivity(strmatch('ala__L_c',Met));
    Alacon_dev = 0;
...
Con = [AAcon;Alacon;TAGcon;DAGcon;MAGcon;Pecon;Pgcon;Ascon;Dgtscon;Dgdgcon;CBcon;Nuccon]';
    Con_dev = [AAcon_dev,Alacon_dev,TAGcon_dev,DAGcon_dev,MAGcon_dev,Pecon_dev,Pgcon_dev,Ascon_dev,Dgtscon_dev,Dgdgcon_dev,CBcon_dev,Nuccon_dev];
