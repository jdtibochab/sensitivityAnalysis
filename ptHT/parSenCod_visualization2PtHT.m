...
load('paralleltestPtHT_interpol.mat')
...
eliminateMets = {'h_c','pi_c','atp_c','h2o_c','starch','dgtp_c','dctp_c','datp_c','dttp_c','utp_c','ctp_c','gtp_c','udprmn_c','pe','pg','pc'};
...
for f = 1:length(fieldNames)
    ...
    for e = 1:length(eliminateMets)
        ...
        if ~isempty(temp)
            group(find(ismember(group,temp))) = [];
        end
    end
...
%             co2_uptake{k} = abs(co2_uptake{k});
            ...
            for j=1:it1 % For all BOFs
                growth_mean(i,j) = mean(growth{k}(:,j)); % Mean value of all points of 1 BOF
...
%                 [poly] = polyfit(parMetCoeff{k}(:,j),co2_uptake{k}(:,j),1);
        ...
        saveas(gcf,['parSenCodCorrConec_Ch_',groupname],'svg')
         ...
         if ~isempty(strmatch('allMets',groupname))
            ...
            save('3D_PtHT.mat','groupConectivity','groupCost','groupSens','groupCod','metTypes','group','Met','growth_sens_vector');
        end
